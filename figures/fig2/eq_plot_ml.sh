...
gmt6 gmtset MAP_FRAME_TYPE plain MAP_FRAME_WIDTH 3p FONT_ANNOT_PRIMARY 9.5p MAP_TICK_LENGTH_PRIMARY 3p MAP_FRAME_PEN 2.2p
...
### Earthquake
awk '{print $1,$2}' $eq_all_LE | gmt6 psxy $Rmap $J -: -Sc.045 -Gblack -B -O -P -K >> $PS #GA catalog all LE eq white circles
# for solid star with white boundary -Sa.32 -W.15,white -Gdarkred
#
gmt6 psxy $Rmap $J -W.75p,navy@5,- -V -O -K << EOF >> $PS
...
awk '{print $1,$2,$3,$4*.14}' $eq_na_ml_F | gmt6 psxy $Rmap $J -: -Sc -W.15,black -Cdark_reds.cpt -t20 -B -O -P -K >> $PS # eq
...
gmt6 pslegend -Dx1.2c/2c+w2.1c/2.1c+o-1c/-.5c -F+gwhite+p.1 -O $J $Rmap << EOF >> $PS
...
S 0.2c c 0.16c white .25p,black 0.2i Mine event
