...
######
for day in range(339,341):
    ### getting coincidence trigger from multi station trigger
    print('--> Doing day #',day)
    with open('../trigger/multi_trig/{}_triggers_pickle_E.txt'.format(day), 'rb') as f:
    ...
    for item in coinc_trigger:
        ...
        if count > 8:
            ...
            item_index=1
            os.mkdir('{}/trig_{}_E'.format(day,folder_index))
            for station in item['stations']:
                if station=='OOD':
                    client = Client("IRIS")
                    OOD=client.get_waveforms("AU", "OOD", "*", "*E", time-120, time + 300)
                    trace=OOD[0]
                    df = trace.stats.sampling_rate
                    ...
                    plt.savefig('{}/trig_{}_E/item_{}.pdf'.format(day,folder_index,item_index), \
                    bbox_inches='tight', pad_inches=0.2)
                elif station=='MAL49':
                    cl = Client('http://auspass.edu.au:80',user='3g', password='orange44')
                    MAL=cl.get_waveforms("3G", "MAL49", "*", "*E", time-120, time + 300)
                    trace=MAL[0]
                    df = trace.stats.sampling_rate
                    trace=bp_taper_dsp(trace)
                    cft1 = recursive_sta_lta_py(trace.data, int(STA * df), int(LTA * df))
                    ...
                    stream= obspy.read('../../2018/{}/{}*0000.EHE'.format(Jday,station))
