...
def plot_trigger_1(trace, cft, thr_on, thr_off, p_pick, show=False):
    import matplotlib.pyplot as plt
    df = trace.stats.sampling_rate
    npts = trace.stats.npts
    t = np.arange(npts, dtype=np.float32) / df
    ...
    ########
    max_trigger_length=40 # max single trigger length in sec
    ...
    try:
        ...
        pass
    ax1.vlines(p_pick, i, j, color='seagreen', lw=1,label="p_pick")
    ax2.axhline(thr_on, color='tomato', lw=1, ls='--')
    ax2.axhline(thr_off, color='skyblue', lw=1, ls='--')
...
# 'multi_rcr_detect.py' and for each day. Then for each item, it
#
#
...
for day in range(335,336):
    ...
    for station in item['stations']:
        if station=='OOD':
            client = Client("IRIS")
            OOD_Z=client.get_waveforms("AU", "OOD", "*", "*Z", time-0, time + 60)[0]
            OOD_N=client.get_waveforms("AU", "OOD", "*", "*N", time-0, time + 60)[0]
            OOD_E=client.get_waveforms("AU", "OOD", "*", "*E", time-0, time + 60)[0]
            OOD_Z=bp_taper_dsp(OOD_Z)
            OOD_N=bp_taper_dsp(OOD_N)
            OOD_E=bp_taper_dsp(OOD_E)
            df = OOD_Z.stats.sampling_rate
            p_pick= ar_pick(OOD_Z.data, OOD_N.data, OOD_E.data, df, \
                4, 19,1.0, 0.1, 4.0, 1.0, 2, 8, 0.1, 0.2,s_pick=False)
            print(p_pick)
            # cft1 = recursive_sta_lta_py(OOD_Z.data, int(.1 * df), int(1 * df))
            cft1 = classic_sta_lta(OOD_Z.data, int(.1 * df), int(1 * df))
            plot_trigger_1(OOD_Z, cft1, On, Off,p_pick,show=False)
            # plt.savefig('{}/trig_{}/item_{}.pdf'.format(day,folder_index,item_index), \
            # bbox_inches='tight', pad_inches=0.2)
            plt.savefig('p_pick_{}.pdf'.format(station), \
            bbox_inches='tight', pad_inches=0.2)
    #     elif station=='MAL49':
