...
*             write(*,*)' start forward modeling iteration: ',it-1
             write(*,*)'forward modeling iteration: ',it-1
	   end if
...
c						from a NAD file
	   ...
	   if(it.eq.1.and.istype.eq.1.and.noforward)go to 31
           t1 = cputime(t2,t3)
	   do 30 i = 1,ns
...
c
 30	   continue
           ...
           ttfor = ttfor + t2
 31        continue
*31        if(summary)write(*,*)' finished forward modeling'
c
...
c       end if
     ...
     &  'Total time for ordering misfits                ',tmis
        write(lu_sum,*)
     &  'Total time for NA-algorithm                    ',tnat
        write(lu_sum,*)
...
c                                               Write header to summary file.
        write(lu_sum,fmt='(/,3x,
     &  "Summary information of Neighbourhood ",
...
c
c       write(lu_out,*)' Perform NA or MC ? (0=NA,1=MC)'
        read(lu_na,*)itype
        monte = .false.
        if(itype.eq.1)then
           monte = .true.
...
c	end if
c
c       write(lu_out,*)' Enter number of axis samples'
c       read(lu_na,*)naxis
c
c
...
c					sample from nad file
        if(istype.eq.1)then
           ...
           len = 16
           open(lu_nad,file='na.nad',status='old',
...
c       write(lu_sum,*)' of distance database          : ',nclean
        ...
        write(lu_sum,*)' Length of walk in cell        : ',nsleep
        write(lu_sum,*)' Random seed value             : ',iseed 
        if(sobol)then
           write(lu_sum,*)' SAS Quasi-random sequence used'
        else
           write(lu_sum,*)' Pseudo-random sequence; seed  : ',iseed
        end if
	if(istype.eq.1)then
...
c       write(lu_out,*)' of distance database          : ',nclean
        ...
        write(lu_out,*)' Length of walk in cell        : ',nsleep
...
c                                               models from a NAD file
        if(istype.eq.1.and..not.monte)then
           call read_nad
     &          (lu_nad,'na.nad',nhi,nhui,nd_nad,nsample,
     ...
     &             (na_models(1,i),nd,rangeo,scales,na_models(1,i))
           ...
           write(*,*)' user specified starting models',
     &               ' not yet implemented'
           stop
...
c	      write(lu_sum,*)' restarting model from ',
c    &        ind_cell,' is',is
...
c ---------------------------------------------------------------------
c
c       write_nad - write a direct access file in NAD format
c
c       Input:
...
c
     ...
     &           (lu,fnme,nd,ne,nh,nhu,header,models,data)
c
      real*4            models(nd,ne)
      real*4            data(ne)
      character         header(nh)
...
c     write(*,*)' length of nad file = ',len
...
c						check array sizes
	...
	if(nh.gt.nhmax)then
 	...
 	call read_da(lu,fnme,nd,ne,nh,header,models,data)
...
c ---------------------------------------------------------------------
c
      	Subroutine read_da(lu,fnme,nd,ne,nh,header,models,data)
c
      	real*4            models(nd,ne)
      	real*4            data(ne)
      	character         header(nh)
...
c					
	len = 4+nd*ne+ne+1
     ...
     &       form='unformatted',access='direct',recl=len)
      	...
      	read(lu,rec=1)i,j,k,kk,header,models,data,tail
	
	close(lu)

	if(tail.ne.-999.0)then
          write(*,*)
          write(*,*)' Error detected in reading nad file '
          write(*,*)'       tail variable is incorrect'
          write(*,*)
	end if
c
      	return
