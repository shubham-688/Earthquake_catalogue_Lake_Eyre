...
c
 ...
 100  format(a,$)
      ...
      subroutine dasign(lu,mode,ia,nc,len)
c
c $$$$$ calls no other routine $$$$$
c
...
c
      ...
      go to (1,2),nf
 1    go to (11,12,13),ns
 11   open(lu,file=ia(1:nc),status='old',form='formatted',
     1 access='direct',recl=len)
      return
 12   inquire(file=ia(1:nc),exist=exst)
      if(exst) go to 11
 13   open(lu,file=ia(1:nc),status='new',form='formatted',
     1 access='direct',recl=len)
      return
 2    go to (21,22,23),ns
 21   open(lu,file=ia(1:nc),status='old',form='unformatted',
     1 access='direct',
     1 recl=len)
      return
 22   inquire(file=ia(1:nc),exist=exst)
      if(exst) go to 21
 23   open(lu,file=ia(1:nc),status='new',form='unformatted',
     1 access='direct',
     1 recl=len)
      return
      end
