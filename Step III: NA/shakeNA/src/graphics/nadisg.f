...
c
       ...
       DO k=0,20
         ...
         IF(msf.eq.0) then
           sk = 0.84-0.04*(k+1)
           call greyton(k+31,0.15+0.45*sk)
           call pen(k+31,0)
           if(k.lt.12) then
             ick = 12+k/2
             sz = 0.09-0.002*k
...
c           write(6,*) n,alat,alon,adep,asec,amisf
           ...
           IF(msf.eq.1) then
             ...
             sk = 0.1+0.7*xm/amc
             call greyton(k+31,sk)
             call pen(k+31,0)
           ENDIF
           if(abs(xsa).lt.5. .and. abs(ysa).lt.5.) 
     &           call csymbl(x2r+xsa,y2r+ysa,3,sz,ick)
           if(abs(xsa).lt.5. .and. abs(ysb).lt.d1) 
     &           call csymbl(x3r+xsa,y3r-ysb,3,sz,ick)
           if(abs(ysa).lt.5. .and. abs(ysb).lt.d1) 
     &           call csymbl(x4r+ysb,y4r+ysa,3,sz,ick)
           if(abs(xsb).lt.6.) 
     &           call csymbl(x1r+xsb,y1r+ysc,3,sz,ick)           
         ENDDO
       ENDDO
c      
       call Hplots(0,0,8,1)
